...
- `HOST` / `PORT`：代理服务器地址（如 `127.0.0.1:3456`）
- `APIKEY`：Claude Code 的认证密钥 - 同时也是 Web UI 的访问密码
- `Providers`：你的上游供应商列表
...
将 `<APIKEY>` 替换为你 `config.json` 中的 `APIKEY` 值。
...
浏览器会弹出 Basic Auth 认证 - 密码就是你的 `APIKEY`（用户名随意）。
