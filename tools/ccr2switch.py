...
用法: python ccr2switch.py [--input config.json] [--output cc-switch.sql]
"""
...
def provider_to_sql(provider, is_current=False, index=None, add_prefix=False):
    ...
    # 提取网站 URL（使用简单的 / 分离方法）
    # 例如: https://api.example.com/v1/messages -> https://api.example.com
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例:
  python ccr2switch.py                                    # 使用默认文件
  python ccr2switch.py --input ccr.in.json                # 指定输入文件
  python ccr2switch.py --current "runanytime.hxi.me"      # 指定当前激活的供应商
  python ccr2switch.py --output my-switch.sql             # 指定输出文件
        """
    )
